;-----------------------------------------------------------------------------
; startup.asm - modified version of 'standard' Huc startup include
;.............................................................................
; for use with Squirrel Sound System. 
;-----------------------------------------------------------------------------
...
_system:
	...
	sta   <psg_inhibit		; mark psg as ready, using timer
...
;----------------------------------------------------------------------------------------
; ths section changed to allow psg driver to run. Modifications have been made so that
; the psg does NOT run continuously. You must call psgOn() to start the psg.
;----------------------------------------------------------------------------------------
	...
	;--------------------------------------------------------------------------
	; is a psg interrupt already being serviced ?
	;--------------------------------------------------------------------------
	
	lda   	<psg_irqflag			; is IRQ being serviced ?
	bne 	.skip_psg				; yes, skip this one
	inc		<psg_irqflag			; lock-out other interrupts

	;--------------------------------------------------------------------------
	; is the psg turned on ?
	;--------------------------------------------------------------------------
	
	lda		<psg_inhibit
	and		#$80					; is bit 7 on ?
	beq		.clear_psg				; nope, psg is OFF
	
;	bbr7		<psg_inhibit, .skip_psg		; bit 7 indicates psg running
											;     1 means psg running
											; so, if psg not turned on, skip it

	;--------------------------------------------------------------------------
	; we are in vsync interrupt. is player using vsync or timer ?
	; if player is using timer interrupt, skip the vsync one.
	;--------------------------------------------------------------------------

	lda		<psg_inhibit
	and		#PSG_IRQ						; isolate interrupt
	beq		.clear_psg						; if on timer, skip it
	
;	bbr0		<psg_inhibit, .skip_psg		; bit 0 indicates irq.
											;     0 means on timer.
											; so this is "if we're on timer, skip it"
											
	;--------------------------------------------------------------------------
	; psg is on and responding to vsync. mark irq as being serviced (to lock
	; out any other irqs), and call the driver.
	...
	;--------------------------------------------------------------------------
	; driver has finished; we should be able to clear our lock
...
;----------------------------------------------------------------------------------------
; this section has been changed to allow psg driver to run. I don't know WHY the psg code
; was already in the timer section (since it was only supposed to work on vsync), but the
; psg driver did not work correctly without it.
;----------------------------------------------------------------------------------------
; ----
; timer
; ----
...
;------------------------------------------------------------------------------------------
; end system portion of psg drver routine.
;------------------------------------------------------------------------------------------
